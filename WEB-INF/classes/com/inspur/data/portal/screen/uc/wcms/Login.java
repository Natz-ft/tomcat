...
public class Login implements ViewHandler {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getSession().getAttribute("userInfo")!=null){
			response.sendRedirect(Function.getLink("wcms/userInfo.jsp"));
			return;
		}
...
//		String RelayState = request.getParameter("RelayState");
		...
		if (session.getAttribute("uid") != null) {
			...
			if (RelayState == null) {
				String default_relay_state = ConfUtil
						.getValue("default_relay_state");
...
//			}
	...
	public void doLogin(HttpServletRequest request, HttpServletResponse response)
		...
		// session中的验证码
		HttpSession session = request.getSession();
		String verify_session = String.valueOf(session
				.getAttribute("verify_code"));
		String login_salt_from_font = String.valueOf(request.getParameter("login_salt"));
		log.error("doLogin  session Id is="+session.getId());
		...
		if (account == null || pwd == null) {
			res = "-1";// 用户名或密码错误
		} else if (login_fail_time >= 1 && !"null".equals(verify_session)
				&& !verify_session.equalsIgnoreCase(verify_request)) {
			res = "-2"; // ret: -2 验证码不匹配
		} else {
			user = UCUtil.getUser(account);
			if (user == null || user.size() == 0
					|| user.get("user_type").toString().equals("91")) {
				res = "-5";// -5:系统中不存在此用户
			} else {
				...
				String is_enable_login_salt = ConfUtil
						.getValue("is_enable_login_salt");
				if ("1".equals(is_enable_login_salt)) {
//					log.error("doLogin seeion value:"+session.getAttribute("login_salt"));login_salt_from_font
//					if (UCUtil.isEmpty(session.getAttribute("login_salt"))) {
					if (UCUtil.isEmpty(login_salt_from_font)) {
						res = "4";
						log.error("login_salt为空，可能的登录重放攻击！");
					} else {
//						String login_salt = String.valueOf(session
//								.getAttribute("login_salt"));
						res = UserUtils.getUserDomain().login(user, pwd,
								login_salt_from_font)
								+ "";
					}
				} else {
					res = UserUtils.getUserDomain().login(user, pwd,
		...
		if (res.equals("1") || res.equals("10")) {
			...
			// ,第三方账号'outacc'
			UCUtil.setCookie("login_type", "userpwd", -1, "/", cookieDomain,
					response);
			...
			UCUtil.setCookie("uc_uid", SecurityUtil.jiamiOld(uid), -1, "/",
					cookieDomain, response);
			UCUtil.setCookie("login_time", PhpUtil.time(), -1, "/",
					cookieDomain, response);
			UCUtil.setCookie("uc_user_id", String.valueOf(user.get("user_id")),
					-1, "/", cookieDomain, response);
			UCUtil.setCookie("uc_nick_name",
					String.valueOf(user.get("nick_name")), -1, "/",
					cookieDomain, response);

			// -----记住登录状态的处理-------
			String remember_login_state = request
					.getParameter("hid_remember_login_state");
			...
			if ("1".equals(remember_login_state)) {
				...
				UCUtil.setCookie("uc_uid_state", SecurityUtil.jiami(uid),
						3600 * 24 * 7, "/", cookieDomain, response);
			}
			...
			if ("1".equals(is_remember_me)) {
				// 登录用户名有效期设置为2周
				UCUtil.setCookie("account", account, 3600 * 24 * 7 * 2, "/",
						cookieDomain, response);
			} else if ("0".equals(is_remember_me)) {
				UCUtil.setCookie("account", null, 0, "/", cookieDomain,
						response);
			} else {
				// 不作处理
			}
	 ...
	 * @throws IOException
	 */
	public void doOtherLogin(HttpServletRequest request,
		...
		// 组装callback URL
		String oauth_callback_url = Function
				.getLink("login/login.do?method=oauthCallBack&type=" + type);
		...
		if ("qq".equals(type)) {
			...
			authorize_url = authorize_url + "?response_type=code"
					+ "&client_id=" + ConfUtil.getValue("qq_appid")
					+ "&scope=get_user_info" + "&state=" + oauth_state
					+ "&redirect_uri="
					+ URLEncoder.encode(oauth_callback_url, "utf-8");
			response.sendRedirect(authorize_url);
		} else if ("sina".equals(type)) {
			String authorize_url = "https://api.weibo.com/oauth2/authorize"
					+ "?response_type=code" + "&client_id="
					+ ConfUtil.getValue("sina_appid")
					+
					// "&scope=all" +
					"&redirect_uri="
					+ URLEncoder.encode(oauth_callback_url, "utf-8");
	 ...
	 * @throws IOException
	 */
	public void doOauthCallBack(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		...
		HttpSession session = request.getSession();
		String oauth_callback_url = String.valueOf(session
				.getAttribute("oauth_callback_url"));

		String type = request.getParameter("type");
		if ("qq".equals(type)) {
			String oauth_state_request = request.getParameter("state");
			String oauth_state_session = String.valueOf(session
					.getAttribute("oauth_state"));
			if (!oauth_state_session.equals(oauth_state_request)) {
				log.error("CSRF攻击");
			}
			// 1、code换取token
			String get_token_url = "https://graph.qq.com/oauth2.0/token"
					+ "?grant_type=authorization_code" + "&client_id="
					+ ConfUtil.getValue("qq_appid") + "&client_secret="
					+ ConfUtil.getValue("qq_appkey") + "&code=" + code
					+ "&redirect_uri="
					+ URLEncoder.encode(oauth_callback_url, "utf-8");
			...
			// );
			content = content.substring("callback(".length(),
					content.length() - 2);
			...
			Map bind = UserUtils.getOutBindDomain()
					.getMapByOpenIdAndType(openid, "qq");
			if (bind != null) {
				...
				// session.setAttribute("developer_id", developer_id);
				// }
				Map userInfo = UserUtils.getUserDomain().getUserByUid(
				...
				// 设置cookie
				String cookieDomain = UCUtil.getCookieDomain();
				UCUtil.setCookie("login_type", "outacc", -1, "/", cookieDomain,
						response);
				UCUtil.setCookie("sso_token", SecurityUtil.jiami(uid), -1, "/",
						cookieDomain, response);
				UCUtil.setCookie("uc_uid", SecurityUtil.jiamiOld(uid), -1, "/",
				...
				UCUtil.setCookie("login_time", PhpUtil.time(), -1, "/",
						cookieDomain, response);
				UCUtil.setCookie("uc_user_id",
						String.valueOf(userInfo.get("user_id")), -1, "/",
						cookieDomain, response);
				UCUtil.setCookie("uc_nick_name",
						String.valueOf(userInfo.get("nick_name")), -1, "/",
						cookieDomain, response);
				...
				response.sendRedirect(Function
						.getLink("index/index.do?method=aftersso"));
				return;
			} else {
				// 如果未绑定，则查询qq用户信息，放入session，然后进入绑定页面，执行绑定。
				// 根据openid、access_token查询用户信息
				String get_user_info_url = "https://graph.qq.com/user/get_user_info"
						+ "?access_token="
						+ access_token
						+ "&oauth_consumer_key="
						+ ConfUtil.getValue("qq_appid")
						+ "&openid="
						+ openid
						+ "&format=json";
				...
				session.setAttribute("other_nick_name",
						qq_user_info.get("nickname"));
			...
			String get_token_url = "https://api.weibo.com/oauth2/access_token"
					+ "?client_id=" + ConfUtil.getValue("sina_appid")
					+ "&client_secret=" + ConfUtil.getValue("sina_appkey")
			...
			if (content != null) {
				...
				Map bind = UserUtils.getOutBindDomain()
						.getMapByOpenIdAndType(openid, "sina");
				if (bind != null) {
					...
					// }
					session.setAttribute("uid", uid);
					Map userInfo = UserUtils.getUserDomain()
							.getUserByUid(Integer.parseInt(uid));
					session.setAttribute("userInfo", userInfo);
					session.setAttribute("login_type", "outacc");

					// 设置cookie
					String cookieDomain = UCUtil.getCookieDomain();
					UCUtil.setCookie("login_type", "outacc", -1, "/",
							cookieDomain, response);
					UCUtil.setCookie("sso_token", SecurityUtil.jiami(uid), -1,
							"/", cookieDomain, response);
					UCUtil.setCookie("uc_uid", SecurityUtil.jiamiOld(uid), -1,
					...
					String get_user_url = "https://api.weibo.com/2/users/show.json"
							+ "?access_token=" + token + "&uid=" + openid;
			...
			response.getWriter().write(
					"<script>alert('暂不支持的第三方账号类型！');</script>");
		}
		return;
	}
	 ...
	 * 登录框第三方登录时跳转页面
	 */
	public void doLoginJump(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
	 ...
	 * @throws IOException
	 */
	public void doIsLoginForDlg(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		boolean isLogin = request.getSession().getAttribute("uid") == null ? false
				: true;
		StringBuilder content = new StringBuilder();
		content.append("<html>");
		content.append("<head>");
		...
		content.append("<script type='text/javascript' src='"
				+ Function.getUrl("js/messenger.js") + "'></script>");
		content.append("</head>");
		content.append("<body>");
		content.append("<script>");
	 ...
	 */
	public void doSamlsso(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("uid") != null) {
			...
			System.out.println(sp_consumer_url);
			//2015-10-06 by ct
			...
			/*
			 * if(sp_consumer_url.contains("172.16.65.207")) { sp_consumer_url =
			 * sp_consumer_url.replace("172.16.65.207", "123.232.100.85"); }
			 */
			...
			String saml_response = SecurityUtil.jiami(user_id + "|" + uid + "|"
					+ PhpUtil.time());
			...
			auto_to_sp.append("<form action='" + sp_consumer_url
					+ "' method=\"post\">");
			auto_to_sp
	 ...
	 */
	@Deprecated
	public void doSamlsso1(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("uid") != null) {
			...
			// 生成SAMLResponse
			String uid = String.valueOf(session.getAttribute("uid"));
			String saml_response = SecurityUtil.jiamiOld(uid + "|"
					+ PhpUtil.time());
			StringBuilder auto_to_sp = new StringBuilder();
			auto_to_sp.append("<html>");
			auto_to_sp.append("<head>");
