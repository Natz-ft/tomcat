...
<%@ page import="org.loushang.internet.exception.AhErrException"%>
...
<%
  Log log = LogFactory.getLog(this.getClass());
	...
	String errMsg = "未知错误", errCauseMsg = "未知";
	...
	if(e1 != null ) {
		...
		if(e2 instanceof AhErrException) {
			errCauseMsg = ((AhErrException)e2).getCauseMsg();
		} else {
			errCauseMsg = e2.toString();
		}
		
		log.error("抱歉，您的访问出错了   "+ errMsg);
		log.error("错误详情   "+ errCauseMsg);
		log.error("错误堆栈信息  "+ AhErrException.getStacktraceMsg(e2, AhErrException.STA_MSG_STR));
...
<title>出错了！</title>
<link href="css/common/errbase.css" rel="stylesheet" type="text/css" />
</head>
<body>
	<div class="Prompt">
				...
				<div class="Prompt_txt">
					...
					<p>
						...
						秒后自动跳转至首页，如果不想等待，直接点击<a
							href="javascript:void(window.location.href = '${homeUrl}')">这里</a>
						跳转<br> 或者 <a href="javascript:void(0);"
							onclick="stopJump(this)">停止</a> 自动跳转
					</p>
				</div>
			</div>
		</div>
