define(function(require, exports, module){
	...
	exports.init = function(){
		...
		$.ajax({
			...
			type: "GET",
			data: {app_id: app_id, app_type: "pc", version: curVersionId},
			dataType: "JSON",
			cache: false,
			success: function(result){
				if(result.code == 1){
					...
					if('pic_info' in curVersion){
						light_two = curVersion['pic_info']['preview'];
					}else{
						light_two = false;
					}
	...
	function initIndividualInfoTpl(){
		...
		if($("#individualInfoTpl").length==0){
			return;
		}
		var htm =template.render("individualInfoTpl", curVersion);
	...
	function ableEdit(){
		...
		$("a[__submitInfoBtn=true]").click(function(){
			...
			$this = $(this);
			var version_id = $this.attr("__version_id");
			var platform_type = $("input[name=platform_type_"+version_id+"]").val();
			...
			if(platform_type == "windows" || platform_type == "mac"){
				if(!file_path){
					layer.tips("请上传安装包", $("#upload_apk_btn_txt_" + version_id), 2);
					return;
				}
			}else{
				layer.msg("创建版本时未保存platform_type", 2, 3);
				return;
			}
			var flag = 1;
			var trimReg = /^(\s*)|(\s*$)/g;
...
//			}
	...
	function setUeditor(){
		require("utils/ueditor/ueditor.config.strategy");
		require("utils/ueditor/ueditor.parse");
		require("utils/ueditor/ueditor.all.min");
		//var uEditors = [];
	...
	function setUpload_local(){
		...
		require("utils/swfupload/swfupload");
		require("utils/swfupload/plugins/swfupload.queue");
		require("utils/swfupload/handlers");
		require("utils/swfupload/fileprogress");
			...
			var defaults  = {
				...
				for(var i in versionids){
					...
					var uploadApkSetting = {
						...
						button_placeholder_id : "upload_apk_btn_" + versionids[i],
						button_width: 106,
						button_height: 29,
						file_types : "*.jar;*.war;*.zip;*.tar;*.gz",
						file_size_limit : "511200",	// 50MB
						custom_settings : {
						...
						upload_progress_handler : function(file, bytesLoaded, bytesTotal){
							...
							$("#"+this.customSettings.upload_apk_size).html("文件大小:"+bytesLoaded);
...
//								$("#"+this.customSettings.upload_apk_size).html("文件大小:" + file.size);
								var filesize = (file.size/1000)/1000 + "MB";
								$("#"+this.customSettings.upload_apk_size).html("文件大小:" + filesize);
