define(function(require, exports, module){ 
	...
	exports.init = function(){
		...
		$.ajax({
			...
			type: "GET",
			data: {app_id: app_id, app_type: "inner", version: curVersionId},
			dataType: "JSON",
			cache: false,
			success: function(result){
				if(result.code == 1){
					...
					for(var i in result.data){
						versionids.push(result.data[i]['feature_id']);
					}*/
					...
					light_one = light_one && curVersion['version_name'] && curVersion['version_number'] && curVersion['app_url'] && curVersion['ext_app_tpl'];
	...
	function initIndividualInfoTpl(){
		...
		if($("#individualInfoTplforInner").length==0){
			return;
		}
		var htm =template.render("individualInfoTplforInner", curVersion);
	...
	function ableEdit(){
		...
		$("a[__submitInfoBtn=true]").click(function(){
			...
			var platform_type = $("#platform_type_"+version_id).val();
			var upgrade_instru = $("#upgrade_instru_" + version_id).val();
			var app_url = $("#app_url_"+version_id).val();
			var ext_app_tpl = $("input[name=ext_app_tpl_"+version_id+"]:checked").val();
		...
		$("a[__submitDetailBtn=true]").click(function(){
			...
			//buildIndividualInfo(version_id);
