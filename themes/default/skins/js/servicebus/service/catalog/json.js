...
function Process(str){
  ...
  try{
    ...
    $id("result-textarea").innerHTML = "<PRE class='CodeContainer'>"+html+"</PRE>";
  }catch(e){
	  html = formatXml(str);
	  $id("result-textarea").innerHTML = "<PRE class='CodeContainer'><xmp>"+html+"</xmp></PRE>";
  }
}
  ...
  if(IsArray(obj)){
    if(obj.length == 0){
      ...
      for(var i = 0; i < obj.length; i++){
    	  if(i>10){
    		  html += '<br/><span>//以上是服务返回的前十条，省略其他记录...</span><br/>';
    		  break;
    	  }
...
function CollapseAllClicked(){
  ...
  TraverseChildren($id("result-textarea"), function(element){
...
function CollapseLevel(level){
  ...
  TraverseChildren($id("result-textarea"), function(element, depth){
...
function EnsureIsPopulated(){
  ...
  if(!$id("result-textarea").innerHTML && !!$id("RawJson").value) Process();
...
function SelectAllClicked(){
  ...
  if(!!range.selectNode)
    ...
    range.selectNode($id("result-textarea"));

  else if(range.moveToElementText)

    range.moveToElementText($id("result-textarea"));

  

  if(!!range.select)

    range.select($id("result-textarea"));
