$(function () {
    ...
    $("#relnetSearch").click(function(){
    	...
    	if( null == searchKey || "" == searchKey || typeof(searchKey) == undefined){	
	...
	$('#relnetSearchKey').keydown(function(event){
    	if(event.keyCode==13){
        	var page =1;
        	var searchKey = $("#relnetSearchKey").val();
        	if( null == searchKey || "" == searchKey || typeof(searchKey) == undefined){	
		...
		success: function(res) {
			...
			if(res!=""&&res!=null){
				...
				for (var i = 0; i < data.records.length; i++) {
					...
					arry_list.push("<li rel='"+obj.id+"'><span onclick='catalogtagreload("+obj.id+")'>"+obj.title+"</span><a href='"+contentPath+"catalog/catalogDetail.htm?cata_id="+obj.id+"'>详情</a></li>");
				}
				$("#searchlist>ul").append(arry_list.join(''));
				if (page == 1&&data.totalRecord>pageSize) {
					searchreloadPage(data.totalRecord);
				}else if(page == 1){
					$("#searchlist .pagination").empty();
				}
			}else{
				if (page == 1) {
					$("#searchlist .pagination").html("暂无数据");
					//reloadPage(0);
				}
			}
		},
...
var searchreloadPage = function(totlePage) {
	$("#searchlist .pagination").pagination(totlePage, {
