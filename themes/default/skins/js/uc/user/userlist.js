        ...
        grid.init({
            ...
            dataTable: {
                ...
                },
                {
                    "targets": [7],
                    "render": function(data, type, full) {
                    	var editurl = haspermission('/user/edituser.htm');
                    	var delurl = haspermission('/user/userlist.do?method=updateUserStatus');
                        var text = "<button style='display:"+editurl+"' class=\"btn btn-primary btn-xs ajaxify\" value=\""+getRootPath()+"/user/edituser.htm?uid="+data+"\"><i class=\"fa fa-pencil\"></i></button>";
                        if(editurl=="none" && delurl == "none"){
                        	text = "--";
                        }
                        return text;
                    }
                ...
                "pageLength": 10,
                "ajax": {
                    "url": getRootPath() + '/user/userlist.do?method=getUserList',
                },
                "order": [[7, "desc"]]
            }
        });
