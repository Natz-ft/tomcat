...
//添加关注话题
function addFollowTopic(o,key){

	$.post(U('weibo/operate/followtopic'),{topicId:key},function(txt){
		txt = eval( '(' + txt + ')' );
		if(txt.code=='12'){
			$(o).parents("li").remove();
...
ui = window.ui ||{
	success:function(message,error,callback){
	...
	confirm:function(o,text,cb){
		if(cb && typeof cb == "string"){
			cb = window[cb];
		}
		var callback = cb || $(o).attr('callback');
		text = text || '确定要做此项操作吗？';
	...
	emotions:function(o){
		...
		$body.live('click',function(event){
			var isAImg = true;
			try{
				var tnodename = event.target.nodeName.toLowerCase();
				var inContent = $.contains($('#emot_content').parent()[0],event.target);
				isAImg = !inContent || (inContent && (tnodename==='a' || tnodename==='img'));
			} catch(e){}
			var isBtn = $.contains(o,event.target);
			if( $(event.target).attr('target_set')!=target_set && !isBtn && isAImg){
				$('#emotions').remove();
			}
		});
	},
	...
	getcom:function(prefix,init_style,init_p,init_c){
		var style = (init_style)?'class="'+init_style+'"':'';
		var html = '<select name="'+prefix+'_province" '+style+'><option value="default">省公司</option></select> <select name="'+prefix+'_city" '+style+'><option value="default">不限</option></select>';
		document.write(html);
		$.getJSON(U('home/XsmUser/getComTree'), function(json){
			var province ='';
			var _default = true;
			for(var i in json){
				var pselected='';
				var cselected='';
				var city='<option value="default">不限</option>';
				if(json[i].com_id==init_p){
					 pselected = 'selected="true"';
					 $sub = json[i].sub_com;
					 for(var m in $sub){
						 cselected = $sub[m].com_id == init_c ? 'selected="true"' : '';
						 city+='<option value="'+$sub[m].com_id+'" '+cselected+'>'+$sub[m].com_name+'</option>';
					 }
					 $("select[name='"+prefix+"_city']").html(city);
					 _default = false;
				}
				province+='<option value="'+json[i].com_id+'" rel="'+i+'" '+pselected+'>'+json[i].com_name+'</option>';
			}
			province = '<option value="default"' + (_default ? 'selected="true"' : '') +'>不限</option>' + province;
			$("select[name='"+prefix+"_province']").live('change',function(){
				var city = '<option value="default">不限</option>';
				var handle =  $(this).find('option:selected').attr('rel');
				if(handle){
					var t =  json[handle].sub_com;
					if(json[handle].sub_com){
						$.each(t,function(j,m){
							city+='<option value='+m.com_id+'>'+m.com_name+'</option>';
						});
					}
				}
				$("select[name='"+prefix+"_city']").html(city);
    ...
    if ((version >= 5.5) && (document.body.filters)){ 
       for(var j=0; j<document.images.length; j++){ 
          ...
          if (imgName.substring(imgName.length-3, imgName.length) == "PNG"){
        	 var src = img.src;
//			 img.src = _THEME_+'/images/s.gif';
//			 debugger;
//			 img.style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="'+src+'",sizingMethod="scale")';
//			 img.style.background='none transparent';//搞定了
          } 
       } 
    }     
}
...
function changeverify(){
  ...
  if(url.match(/\d{13}\d{10}/i)){
  	url = url.replace(/[\d{13}\d{10}]/i, ttime);
  }else{
  	url += url.indexOf("?")!=-1 ? "&"+ttime : "?"+ttime;
  }
...
function updateCount(txt){
    ...
    var list = {
                     comment:{url:U('home/user/comments'),name:"我的评论"},
                     atme:{url:U('home/user/atme'),name:"新@提到我"},
                     message:{url:U('home/message/index'),name:"新的站内信"},
                     group_atme:{url:U('group/index/atme'),name:"群内@我的"},
                     group_comment:{url:U('group/index/comment'),name:"群内评论"},
                     group_bbs:{url:U('group/index/bbsNotify'),name:"群内帖子消息"},
                     notify:{url:U('home/message/notify'),name:"系统通知"},
	   ...
	   var isshow = false;
	    for(var one in list){
	        ...
	        if(parseInt(a) >0){
	            ...
	            //<li>两条新消息,<a href="#">查看消息</a></li>
	            var newLi = document.createElement('li');
	            var newNode = document.createTextNode(a+"条"+list[one].name+"，");
	            var newA = document.createElement('a');
	            newA.href = list[one].url;
	            newA.target = "";
	            newA.innerHTML="查看消息";
	...
	if(txt && txt.comment != undefined && txt.comment!="0"){
		$("#app_left_count_comment").html("(<font color=\"red\">"+txt.comment+"</font>)");
	}else{
	     $("#app_left_count_comment").html("");
	}
	
	if(txt && txt.atme != undefined && txt.atme!="0"){
		$("#app_left_count_atme").html("(<font color=\"red\">"+txt.atme+"</font>)");
	}else{
	    $("#app_left_count_atme").html("");
	}
