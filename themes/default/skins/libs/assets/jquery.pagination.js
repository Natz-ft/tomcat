...
jQuery.fn.pagination = function(maxentries, opts){
	...
	opts = jQuery.extend({
		...
		whether_select:false,//是否生成下拉框分頁
	...
	return this.each(function() {
		 ...
		 */
		function numPages() {
			return Math.ceil(maxentries/opts.items_per_page);//返回总页数
		}
		...
		function drawLinks() {
			...
			// update
			console.log(opts.whether_select)
			if(opts.whether_select)
			{
				var options = "";
				...
				var content = "<select class='page-select'>"+options+"</select>";
				panel.append(content);
				$("body").on("change",".page-select",function(evt){
					var page_id = $(this).val()-1;
					current_page = page_id;
					drawLinks();
					var continuePropagation = opts.callback(page_id, panel);
					if (!continuePropagation) {
