...
<website:style href="css-open/dev/app-detail.css"/>
	...
	<script type="text/html" id="appInnerTpl" >
			<div class="cont-title-bold clearfix">
				<span class="title">应用名称：<?=app.app_alias?></span>
				<div class="opt">
					<a class="btn-add-version" style="color:#fff;" href="#inner/create">添加新版本</a>
				</div>
			</div>
			<ul class="app-versions">
				...
				<li class="app-version" id="version_<?=list[i]['feature_id']?>">
						...
						<?}else{?>
							...
							<span class="version_name"><?=list[i]['version_name']?></span>
							<a class="btn-drop" href="#inner!version=<?=list[i]['feature_id']?>">[查&ensp;看]</a>
							<?if(list[i]['app_status'] == 1){?>
								<span class="app_status status-warning"><i class="icon"></i>已提交</span>
							<?}else if(list[i]['app_status'] == 2){?>
								<span class="app_status status-success"><i class="icon"></i>已上线</span>
								...
								<ul class="drop-detail-cates">
									<li class="drop-detail-cate drop-detail-cate-basic">
										...
										<div class="cate-body">
											...
											<dl class="clearfix">
												<dt><em class="required">*</em>实际地址：</dt>
												<dd>
											...
											<dl class="clearfix">
												<dt>iframe宽度：</dt>
												<dd>
													<input type="radio" name="ext_app_tpl_<?=curVersion['feature_id']?>"  value="800" <?if(!curVersion['ext_app_tpl'] || curVersion['ext_app_tpl'] == 800){?> checked <?}?> /> 800px
													<input type="radio" name="ext_app_tpl_<?=curVersion['feature_id']?>" value="950" <?if(curVersion['ext_app_tpl'] == 950){?> checked <?}?> /> 950px
													
													<p class="form-common-tip">
														应用页面的宽度最宽不宜超过此宽度，超过部分将不会被显示，高度自适应
													</p>
												</dd>
											</dl>
											<dl class="clearfix">
				...
				<ul class="service-list">
					...
					<a href="#inner/create" style="color:#23B3DA;">还没有创建任何版本,赶紧去创建一个</a>
					</li>
				</ul>
				<?}?>
			</ul>
...
<website:widget path="dev/open/app/individualInfoTplforInner.jsp" />
