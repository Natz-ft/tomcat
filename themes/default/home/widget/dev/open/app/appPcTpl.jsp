...
<website:style href="css/dev/form.css"/>
<website:style href="css-open/dev/app-detail.css"/>
<script type="text/html" id="appPcTpl" >
			<div class="cont-title-bold clearfix">
				<span class="title">应用名称：<?=app.app_alias?></span>
				<div class="opt">
					<a class="btn-add-version" style="color:#fff;"  href="#pc/create">添加新版本</a>
				</div>
			</div>
			<ul class="app-versions">
				...
				<li class="app-version" id="version_<?=list[i]['feature_id']?>">
						...
						<?if(list[i]['feature_id'] == curVersion['feature_id']){?> <!--当前版本-->
							...
							<span class="platform_type">
								<?if(list[i]['platform_type'] == 'windows'){?>
									<i class="icon-ipad-black"></i>windows
								<?}?>
								<?if(list[i]['platform_type'] == 'mac'){?>
									<i class="icon-iphone-black"></i>mac
								<?}?>
								<?if(list[i]['platform_type'] == 'linux'){?>
									<i class="icon-ipad-black"></i>linux
								<?}?>
							</span>
						...
						<?}else{?>
							...
							<span class="version_name"><?=list[i]['version_name']?></span>
							<a class="btn-drop" href="#pc!version=<?=list[i]['feature_id']?>">[查&ensp;看]</a>
							<span class="platform_type">
								<?if(list[i]['platform_type'] == 'windows'){?>
									<i class="icon-ipad-black"></i>windows
								<?}?>
					...
					<div class="drop-detail form-body" id="form_body_<?=curVersion['feature_id']?>">
								...
								<ul class="drop-detail-cates">
									<li class="drop-detail-cate drop-detail-cate-basic">
										...
										<div class="cate-body">
											...
											<!--上传应用安装包-->
											<dl name="feature_type_acess" style="display:block;"  id="feature_install_pg_<?=curVersion['feature_id']?>" >
												<dt><em class="required">*</em>安装包：</dt>
												<dd>
													<p style="height:36px; display:block; position:relative">
														<span style="position:absolute;display:block; z-index:1;"><b id="upload_apk_btn_<?=curVersion['feature_id']?>"></b></span>
													...
													<p class="form-common-tip" style="width:360px;">
															<span id="upload_apk_status_<?=curVersion['feature_id']?>">
															<? if(curVersion['file_path']) {?>
																文件名: <?=curVersion['file_name']?>
															<?}else{?>
																请上传应用安装包,支持.jar,.war,.zip,.tar,.gz格式
															<?}?>
															</span>
						...
						<dl class="clearfix">
							<dt>当前操作：</dt>
							<dd>
								<!--<a class="btn_sgray mr" href="javascript:void(0)">关&emsp;闭</a>
								
								<a class="btn_sgray" href="javascript:void(0)" __deleteVersionBtn="true" __version_id="<?=list[i]['feature_id']?>">删&emsp;除</a>-->
				...
				<ul class="service-list">
					...
					<a href="#pc/create" style="color:#23B3DA;">还没有创建任何版本,赶紧去创建一个</a>
					</li>
				</ul>
				<?}?>
			</ul>
