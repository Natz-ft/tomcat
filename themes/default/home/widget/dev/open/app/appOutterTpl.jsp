...
<website:style href="css-open/dev/app-detail.css"/>
	<script type="text/html" id="appOutterTpl" >
			<div class="cont-title-bold clearfix">
				<span class="title">应用名称：<?=app.app_alias?></span>
				<div class="opt">
					<a class="btn-add-version" style="color:#fff;" href="#outter/create">添加新版本</a>
				</div>
			</div>
			<ul class="app-versions">
				...
				<li class="app-version" id="version_<?=list[i]['feature_id']?>">
						...
						<?}else{?>
							...
							<span class="version_name"><?=list[i]['version_name']?></span>
							<a class="btn-drop" href="#outter!version=<?=list[i]['feature_id']?>">[查&ensp;看]</a>
							<?if(list[i]['app_status'] == 1){?>
								<span class="app_status status-warning"><i class="icon"></i>已提交</span>
							<?}else if(list[i]['app_status'] == 2){?>
								<span class="app_status status-success"><i class="icon"></i>已上线</span>
									...
									<li class="drop-detail-cate drop-detail-cate-basic">
										...
										<div class="cate-body">
											...
											<dl class="clearfix">
												...
												<dd>
													<input class="input" name="platform_type" value="<?=curVersion['platform_type']?>" type="hidden"  id="platform_type_<?=curVersion['feature_id']?>" style="height:25px"/>
											...
											<dl class="clearfix">
												<dt><em class="required">*</em>访问地址：</dt>
												<dd>
													<input class="input" name="app_url" value="<?=curVersion['app_url']?>" type="text"  id="app_url_<?=curVersion['feature_id']?>" style="height:25px"/>
													<p class="form-common-tip">
														请填写网站域名，例如：http://127.0.0.1:8080/index.jsp 
													</p>
												</dd>
											</dl>
											
											<dl class="clearfix">
												<dt><em class="required">*</em>访问方式：</dt>
												<dd>
													<input style="margin-right:5px;" type="radio" name="website_app_open_type_<?=curVersion['feature_id']?>" tempvalue="<?=curVersion['website_app_open_type']?>" value="0" checked="checked">在新窗口中打开应用链接 
													<input style="margin-left: 20px;margin-right:5px;" type="radio" name="website_app_open_type_<?=curVersion['feature_id']?>" tempvalue="<?=curVersion['website_app_open_type']?>" value="1">在当前窗口中打开应用链接
													<p class="form-common-tip">
														请填写网站打开方式 
													</p>
												</dd>
				...
				<ul class="service-list">
					...
					<a href="#outter/create" style="color:#23B3DA;">还没有创建任何版本,赶紧去创建一个</a>
					</li>
				</ul>
				<?}?>
			</ul>
...
<website:widget path="dev/open/app/individualInfoTplforOutter.jsp" />
