$(function () {
	...
	$("#relalist li span").live("click",function() {
		var id = $(this).parent().attr('rel');
		if(id!=""){
			getcatalogtaglist(id,"",1,"");
			$("#cata_id").val(id);
			$("#sec2 option[value='"+id+"']").attr("selected","selected");
			$("#cata_title").val("");
		}
	});
	//加载下拉框数据
	...
	option2 = {
			tooltip: {trigger: 'item',formatter: '{a} : {b}'},
			legend: {x: 'right',y: 3,data: ['关联数据集']},
			series: []
	};
	//加载图谱及相关列表数据
	//id:数据集编码，title：数据集标题 type：请求类型（0：初始化，1：标签数据集图谱；2：数据集图谱，tag：标签）
	var getcatalogtaglist = function(id,title,type,tag) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					allOption.series.push({
						type: 'force',
						name: "关联",
						categories:[{name:'根数据集',itemStyle:{normal:{color:'#008dd5',label:{show:true}},emphasis:{color:'#63B8FF'}}},{name:'标签',symbol:'triangle',itemStyle:{normal:{label:{}}}},{name:'关联数据集',itemStyle:{normal:{color:'#00CD66'},emphasis:{color:'#00EE76'}}},{name:'根标签',symbol:'triangle',itemStyle:{normal:{color:'#008dd5'},emphasis:{color:'#63B8FF'}}}],
						itemStyle:{normal:{label:{show:false,textStyle:{color:'#333'}},nodeStyle:{brushType:'both',strokeColor:'rgba(255,215,0,0.4)',lineWidth:1}},emphasis:{label:{show:false},nodeStyle:{},linkStyle:{}}},
						useWorker: false,
						minRadius: 13,
						maxRadius: 25,
						coolDown: 0.995,
					...
					$("#wangluotp").click(function(event) {
						...
						$("#kaiguan").bind("click",function(event) {
							if(mmmn==0){
								...
								myChart.setOption({series : [{itemStyle:{normal:{label:{show:true}}}}]});
								mmmn=1;
							}
							else if(mmmn==1){
								$("#kaiguan").addClass('title_off');
								$("#kaiguan").removeClass('title_on');
								myChart.setOption({series : [{itemStyle:{normal:{label:{show:false}}}}]});
								mmmn=0;
							}
						});	
						...
						$("#fenbutp").click(function(event) {
							...
							$("#kaiguan").bind("click",function(event) {
								if(mmm==0){
									$("#kaiguan").addClass('title_on');
									$("#kaiguan").removeClass('title_off');
									myChart2.setOption({series : [{itemStyle:{normal:{label:{show:true}}}}]});
									mmm=1;
								}
								else if(mmm==1){
									$("#kaiguan").addClass('title_off');
									$("#kaiguan").removeClass('title_on');
									myChart2.setOption({series : [{itemStyle:{normal:{label:{show:false}}}}]});
									mmm=0;
								}
							});	
						});
			...
			success: function(data) {
				if(data!=""&&data!=null){
					...
					for (var i = 0; i < data.records.length; i++) {
						var obj = data.records[i];
						if(obj.cata_id!=id){
							arry_list.push("<li rel='"+obj.cata_id+"'><span>"+obj.cata_title+"</span><a href='"+contentPath+"catalog/detail.htm?cata_id="+obj.cata_id+"'>详情</a></li>");
						}
					}
