...
$("#rela_wdinput").click(function(event) {
	$("#rela_xz").show();
	$("#rela_xz").addClass("rela_dedown");
	$(".rela_wdlist").show();
	//$(".rela_wdlist").slideDown();
});
...
$(document).ready(function(){
	...
	$(document).click(function(){ 
		if(i==0){
			$("#rela_xz").hide();
			$(".rela_wdlist").hide();
			//$("#rela_xz").removeClass("rela_dedown");
		}
	});
});
...
$("#rela_qd").click(function(){
	var relactionCondition = "";
	$("input[name='reactionConditions']:checked").each(function(){
		relactionCondition = relactionCondition + $(this).val()+",";
	});
	if("" == relactionCondition){
		easyDialog.open({
			container : {
				content : '请先选择关联维度'
			},
	...
	$("#rela_wdinput").val(relactionCondition.substring(0,relactionCondition.length-1));
	$("#rela_xz").hide();
	$(".rela_wdlist").hide();
	//$(".rela_wdlist").slideUp();
	//$("#rela_xz").removeClass("rela_dedown");
...
$("#queryByfilter").click(function(){
	var param = "";
	var filterName = $("#countclass").val();
	if("0" == filterName){
		var filterValue = $("#tagCount").val();
		param = param + "filterName=tagBase&filterValue="+filterValue;
	}else if("1" == filterName){
		var filterValue = $("#relationCount").val();
		param = param + "filterName=tagLat&filterValue="+filterValue;
	}else {
		var filterValue = $("#orderType").val();
...
$("#queryByRelaction").click(function(){
	...
	var relactionCondition = $("#rela_wdinput").val();
	...
	if(""!=relactionCondition && null != relactionCondition && typeof(relactionCondition) != undefined && "不限" !=relactionCondition){
		param = param + "&relactionCondition="+relactionCondition;
	}
//	$("#rela_page").hide();
//	$("#addList").html("<span>请先添加筛选结果</span>");
//	$("#minuList").html("")
	...
	$.ajax({
	    ...
	    success: function(data){
	    	if(null == data || "null" == data){
	    		...
	    		$("#addList").html(html);
	    		$("#totalRecode").html("共"+data["totalRecord"]+"条数据集");
	    		$("#pageNumber").html(page+"/"+data["totalPage"]);
	    		$("#page").val(page);
