...
(function() {
	...
	var cutBoundsInRange = function(bounds) {
		...
		return new BMap.Bounds(new BMap.Point(minX, minY), new BMap.Point(maxX,
				maxY));
	...
	MarkerClusterer.prototype._createClusters = function() {
		...
		var extendedBounds = getExtendedBounds(this._map, mapBounds,
				this._gridSize);
	...
	Cluster.prototype.addMarker = function(marker) {
		...
		if (!this._center) {
			...
			if (this._isAverageCenter) {
				...
				var lat = (this._center.lat * (l - 1) + marker.getPosition().lat)
						/ l;

				var lng = (this._center.lng * (l - 1) + marker.getPosition().lng)
						/ l;
